...
const (
	...
	IMCEventTypeCrossChainTransactionAdded     InterChainMessageEventType = 50001
	IMCEventTypeCrossChainTransactionCompleted InterChainMessageEventType = 50002
	IMCEventTypeCrossChainTransactionSubmitted InterChainMessageEventType = 50003
	IMCEventTypeCrossChainTransactionClaimed   InterChainMessageEventType = 50004
	IMCEventTypeCrossChainTransactionRejected  InterChainMessageEventType = 50005
	IMCEventTypeCrossChainTransactionRejectReq InterChainMessageEventType = 50006
	IMCEventTypeCrossChainExternalCall         InterChainMessageEventType = 50007
)
...
func ParseToCrossChainTFuelTokenLockedEvent(icme *InterChainMessageEvent) (*CrossChainTFuelTokenLockedEvent, error) {
	...
	logger.Infof("Cross-Chain TFuel Token Locked Event: %v", event)
...
type CrossChainExternalCallEvent struct {
	Chainid         *big.Int
...
func ParseToCrossChainExternalCallEvent(icme *InterChainMessageEvent) (*CrossChainExternalCallEvent, error) {
	if icme.Type != IMCEventTypeCrossChainTransactionAdded {
		return nil, fmt.Errorf("invalid inter-chain message event type: %v", icme.Type)
	}

	var event CrossChainExternalCallEvent
	contractAbi, err := abi.JSON(strings.NewReader(string(scta.BridgeCoreABI)))
	if err != nil {
		return nil, err
	}
	contractAbi.UnpackIntoInterface(&event, "ExternalCall", icme.Data)

	return &event, nil
}

type CrossChainTransactionAddedEvent struct {
	Index         *big.Int
	TxHash        [32]byte
	ChainId       *big.Int
	SignedTxData  []byte
...
func ParseToCrossChainTransactionAddedEvent(icme *InterChainMessageEvent) (*CrossChainTransactionAddedEvent, error) {
	if icme.Type != IMCEventTypeCrossChainTransactionAdded {
		return nil, fmt.Errorf("invalid inter-chain message event type: %v", icme.Type)
	}

	var event CrossChainTransactionAddedEvent
	contractAbi, err := abi.JSON(strings.NewReader(string(scta.MainABI)))
	if err != nil {
		return nil, err
	}
	contractAbi.UnpackIntoInterface(&event, "TransactionAdded", icme.Data)

	return &event, nil
}

type CrossChainTransactionSubmittedEvent struct {
	Index   *big.Int
	TxHash  common.Hash
	Success bool
	Sender  common.Address
...
func ParseToCrossChainTransactionSubmittedEvent(icme *InterChainMessageEvent) (*CrossChainTransactionSubmittedEvent, error) {
	if icme.Type != IMCEventTypeCrossChainTransactionSubmitted {
		return nil, fmt.Errorf("invalid inter-chain message event type: %v", icme.Type)
	}

	var event CrossChainTransactionSubmittedEvent
	contractAbi, err := abi.JSON(strings.NewReader(string(scta.MainABI)))
	if err != nil {
		return nil, err
	}
	contractAbi.UnpackIntoInterface(&event, "TransactionSubmitted", icme.Data)

	return &event, nil
}

type CrossChainTransactionCompletedEvent struct {
	Index   *big.Int
	txHash  common.Hash
	Success bool
	Sender  common.Address
...
func ParseToCrossChainTransactionCompletedEvent(icme *InterChainMessageEvent) (*CrossChainTransactionCompletedEvent, error) {
	if icme.Type != IMCEventTypeCrossChainTransactionCompleted {
		return nil, fmt.Errorf("invalid inter-chain message event type: %v", icme.Type)
	}

	var event CrossChainTransactionCompletedEvent
	contractAbi, err := abi.JSON(strings.NewReader(string(scta.MainABI)))
	if err != nil {
		return nil, err
	}
	contractAbi.UnpackIntoInterface(&event, "TransactionCompleted", icme.Data)

	return &event, nil
}

type CrossChainTransactionClaimedEvent struct {
	Index        *big.Int
	NewValidator common.Address
	Sender       common.Address
}

func ParseToCrossChainTransactionClaimedEvent(icme *InterChainMessageEvent) (*CrossChainTransactionClaimedEvent, error) {
	if icme.Type != IMCEventTypeCrossChainTransactionClaimed {
		return nil, fmt.Errorf("invalid inter-chain message event type: %v", icme.Type)
	}

	var event CrossChainTransactionClaimedEvent
	contractAbi, err := abi.JSON(strings.NewReader(string(scta.MainABI)))
	if err != nil {
		return nil, err
	}
	contractAbi.UnpackIntoInterface(&event, "TransactionClaimed", icme.Data)

	return &event, nil
}

type CrossChainTransactionRejectReqEvent struct {
	Index  *big.Int
	Sender common.Address
}

func ParseToCrossChainTransactionRejectReqEvent(icme *InterChainMessageEvent) (*CrossChainTransactionRejectReqEvent, error) {
	if icme.Type != IMCEventTypeCrossChainTransactionRejectReq {
		return nil, fmt.Errorf("invalid inter-chain message event type: %v", icme.Type)
	}

	var event CrossChainTransactionRejectReqEvent
	contractAbi, err := abi.JSON(strings.NewReader(string(scta.MainABI)))
	if err != nil {
		return nil, err
	}
	contractAbi.UnpackIntoInterface(&event, "TransactionRejectReq", icme.Data)
	return &event, nil
}

type CrossChainTransactionRejectEvent struct {
	Index  *big.Int
	Sender common.Address
}

func ParseToCrossChainTransactionRejectEvent(icme *InterChainMessageEvent) (*CrossChainTransactionRejectEvent, error) {
	if icme.Type != IMCEventTypeCrossChainTransactionRejected {
		return nil, fmt.Errorf("invalid inter-chain message event type: %v", icme.Type)
	}

	var event CrossChainTransactionRejectEvent
	contractAbi, err := abi.JSON(strings.NewReader(string(scta.MainABI)))
	if err != nil {
		return nil, err
	}
	contractAbi.UnpackIntoInterface(&event, "TransactionRejected", icme.Data)
	return &event, nil
}
