...
type MetachainWitness struct {
	...
	mainChainBridgeCore           *scta.BridgeCore       // the BridgeCore contract deployed on the mainchain
	mainChainBridgeCoreAddr       common.Address
...
func NewMetachainWitness(db database.Database, updateInterval int, interChainEventCache *siu.InterChainEventCache, transactionCache *siu.TransactionCache, chanTransaction chan scom.ChanTransaction) *MetachainWitness {
	...
	mainChainBridgeCoreAddr := common.HexToAddress(viper.GetString(scom.BridgeCore))
	mainChainBridgeCore, err := scta.NewBridgeCore(mainChainBridgeCoreAddr, mainchainEthRpcClient)
	...
	mw := &MetachainWitness{
		...
		mainChainBridgeCoreAddr:       mainChainBridgeCoreAddr,
		mainChainBridgeCore:           mainChainBridgeCore,
		...
		transactionChannel:   chanTransaction,
...
func (mw *MetachainWitness) SetSubchainTokenBanks(ledger score.Ledger) {
	...
	mw.subchainqueueaddr = common.HexToAddress(viper.GetString(scom.SubchainQueue))
	mw.subchainqueue, err = scta.NewMain(mw.subchainqueueaddr, mw.subchainEthRpcClient)
	if err != nil {
		logger.Fatalf("failed to set the Subchainqueue contract: %v\n", err)
	}
...
func (mw *MetachainWitness) collectInterChainMessageEventsOnMainchain() {
	mw.collectInterChainMessageEventsOnChain(mw.mainchainID, mw.mainchainEthRpcUrl, mw.mainchainTFuelTokenBankAddr, mw.mainchainTNT20TokenBankAddr, mw.mainchainTNT721TokenBankAddr, mw.mainchainTNT1155TokenBankAddr, mw.subchainqueueaddr, mw.mainChainBridgeCoreAddr)
}

func (mw *MetachainWitness) collectInterChainMessageEventsOnSubchain() {
	mw.collectInterChainMessageEventsOnChain(mw.subchainID, mw.subchainEthRpcUrl, mw.subchainTFuelTokenBankAddr, mw.subchainTNT20TokenBankAddr, mw.subchainTNT721TokenBankAddr, mw.subchainTNT1155TokenBankAddr, mw.subchainqueueaddr, mw.mainChainBridgeCoreAddr)
}

func (mw *MetachainWitness) collectInterChainMessageEventsOnChain(queriedChainID *big.Int, ethRpcUrl string,
	tfuelTokenBankAddr common.Address, tnt20TokenBankAddr common.Address, tnt721TokenBankAddr common.Address, tnt1155TokenBankAddr common.Address, subchainqueueaddr common.Address, bridgecoreaddr common.Address) {
	...
	events := siu.QueryInterChainEventLog(queriedChainID, fromBlock, toBlock, tfuelTokenBankAddr, tnt20TokenBankAddr, tnt721TokenBankAddr, tnt1155TokenBankAddr, subchainqueueaddr, bridgecoreaddr, mw.queryTopics, ethRpcUrl, mw.transactionCache, mw.transactionChannel)
...
func (mw *MetachainWitness) getBlockScanStartingHeight(queriedChainID *big.Int) *big.Int {
	...
	eventTypes := []score.InterChainMessageEventType{
		...
		//score.IMCEventTypeCrossChainTransactionAdded,
	}
...
func (mw *MetachainWitness) getMainchainMaxProcessedNonceEventHeight(icmeType score.InterChainMessageEventType) *big.Int {
	...
	case score.IMCEventTypeCrossChainTransactionAdded:
...
func (mw *MetachainWitness) getSubchainMaxProcessedNonceEventHeight(icmeType score.InterChainMessageEventType) *big.Int {
	...
	case score.IMCEventTypeCrossChainTransactionAdded:
		maxProcessedNonce, err = mw.subchainqueue.GetMaxProcessedTransactionNonce(nil)
		if err != nil {
			break
		}
		eventHeight, err = mw.subchainqueue.GetTransactionEventHeight(nil, maxProcessedNonce)
	case score.IMCEventTypeCrossChainTokenLockTFuel:
