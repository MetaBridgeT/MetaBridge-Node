...
type Orchestrator struct {
	...
	bridgecoreaddr                common.Address
	bridgecore                    *scta.BridgeCore
	// The subchain
	subchainID                   *big.Int
	...
	subchainqueueaddr            common.Address
	subchainqueue                *scta.Main
	// Inter-chain messaging
	interChainEventCache *siu.InterChainEventCache
	TransactionCache     *siu.TransactionCache
	TransactionChannel   chan scom.ChanTransaction
...
func NewOrchestrator(db database.Database, updateInterval int, interChainEventCache *siu.InterChainEventCache, TransactionCache *siu.TransactionCache, TransactionChannel chan scom.ChanTransaction,
	...
	if err != nil {
		logger.Fatalf("failed to create BridgeCore contract %v\n", err)
	}
...
func (oc *Orchestrator) SetLedgerAndSubchainTokenBanks(ledger score.Ledger) {
	...
	oc.subchainqueueaddr = common.HexToAddress(viper.GetString(scom.SubchainQueue))
	oc.subchainqueue, err = scta.NewMain(oc.subchainqueueaddr, oc.subchainEthRpcClient)
...
func (oc *Orchestrator) mainloop(ctx context.Context) {
	...
	go func() {
		for {
			select {
			case item := <-oc.TransactionChannel:

				switch item.Action {
				case "Added":
					oc.handleAdded(item)
				case "External":
					oc.handleExternal(item)
				case "Voted":
					oc.handleVoted(item)
					/**	case "Submitted":
						go oc.handleSubmitted(item)
					case "Completed":
						go oc.handleCompleted(item)
					case "RejectReq":
						go oc.handleRejectReq(item)
					case "Rejected":
						go oc.handleRejected(item)
					case "Claimed":
						go oc.handleClaimed(item)**/
				default:
					logger.Infof("Unknown action: %s", item.Action)
				}
			case <-ctx.Done():
				return
			}
	...
	for {
		...
		case <-oc.eventProcessingTicker.C:
			...
			// Handle Transactions
			oc.processNextTransactionEvent(oc.mainchainID, oc.subchainID) // send transaction from the mainchain to the subchain
		}
	}
}

func (oc *Orchestrator) handleExternal(item scom.ChanTransaction) {
	logger.Infof("handler external is called")
	currentTransaction, _ := oc.TransactionCache.Get(item.Index)
	if currentTransaction == nil {
		logger.Errorf("Transaction %d not found in cache", item.Index)

	}
	logger.Infof("the chainID is %s", currentTransaction.ChainId)
	txOpts, err := oc.buildTxOpts(oc.subchainID, oc.subchainEthRpcClient)
	tx, err := oc.subchainqueue.QueueTransaction(txOpts, currentTransaction.ChainId, currentTransaction.SignedTxData, currentTransaction.Target, currentTransaction.Value, currentTransaction.Index)
	if err != nil {
		logger.Errorf("failed to queue transaction: %v\n", err)
	}
	logger.Infof("the tx is %v", tx)

}

func (oc *Orchestrator) handleAdded(item scom.ChanTransaction) {
	currentTransaction, _ := oc.TransactionCache.Get(item.Index)
	logger.Infof("Voting has started for transaction %d", item.Index)
	if currentTransaction == nil {
		logger.Errorf("Transaction %d not found in cache", item.Index)
	}
	logger.Infof("the rpc url is %s", currentTransaction.RpcUrl)
	txOpts, err := oc.buildTxOpts(oc.subchainID, oc.subchainEthRpcClient)
	tx, err := oc.subchainqueue.Vote(txOpts, currentTransaction.Index, true)
	if err != nil {
		logger.Errorf("failed to vote transaction: %v\n", err)
	}
	logger.Infof("the tx is %v", tx)

}

func (oc *Orchestrator) handleVoted(item scom.ChanTransaction) {
	currentTransaction, _ := oc.TransactionCache.Get(item.Index)
	...
	logger.Infof("the rpc url is %s", currentTransaction.RpcUrl)
	if currentTransaction.Validator == oc.privateKey.PublicKey().Address() {
		currentRpcClient, err := ec.Dial(currentTransaction.RpcUrl)

		if err != nil {
			logger.Errorf("Here i will do the reject thing")
		}
		txOpts, err := oc.buildTxOpts(currentTransaction.ChainId, currentRpcClient)
		if err != nil {
			logger.Errorf("Here i will do the reject thing2")
		}
		logger.Infof("the proxy contract is %v %v %v  %v ", currentTransaction.ProxyContract, currentTransaction.SignedTxData, currentTransaction.Value, currentTransaction.Index)
		proxyContract, err := scta.NewProxyContract(currentTransaction.ProxyContract, currentRpcClient)
		if err != nil {
			logger.Errorf("Here i will do the reject thing3 %v", err)
		}

		tx, err := proxyContract.Execute(txOpts, currentTransaction.Target, currentTransaction.SignedTxData, currentTransaction.Value, currentTransaction.Index)
		logger.Infof("Transaction %d submitted to the proxy contract", tx)
		if err != nil {
			logger.Errorf("Here i will do the reject thing4 %v", err)
		}
		return
	}
...
func (oc *Orchestrator) processNextTransactionEvent(sourceChainID *big.Int, targetChainID *big.Int) {
	oc.proccessNextTransactionAddedEvent(sourceChainID, targetChainID)
	//oc.proccessNextTransactionConfirmedEvent(sourceChainID, targetChainID)
	//oc.proccessNextTransactionSubmittedEvent(sourceChainID, targetChainID)
	//	oc.proccessNextTransactionRejectedEvent(sourceChainID, targetChainID)
	//	oc.processNextTransactionRejectReqEvent(sourceChainID, targetChainID)
	//	oc.processNextTransactionClaimedEvent(sourceChainID, targetChainID)
}

func (oc *Orchestrator) proccessNextTransactionAddedEvent(sourceChainID *big.Int, targetChainID *big.Int) {
...
func (oc *Orchestrator) processNextEvent(sourceChainID *big.Int, targetChainID *big.Int, sourceChainEventType score.InterChainMessageEventType, maxProcessedNonce *big.Int) {
	...
	if err == ts.ErrKeyNotFound {
		...
		logger.Infof("we found the event")
	}

	logger.Infof("Process next event, sourceChainID: %v, targetChainID: %v, sourceChainEventType: %v, nextNonce: %v",
...
func (oc *Orchestrator) mintTFuelVouchers(txOpts *bind.TransactOpts, targetChainID *big.Int, sourceEvent *score.InterChainMessageEvent) (common.Hash, error) {
	...
	logger.Infof("mintTFuelVouchers, dynasty: %v, targetChainID: %v, denom: %v, tokenLockNonce: %v , lockedamount: %v", dynasty, targetChainID, se.Denom, se.TokenLockNonce, se.LockedAmount)
...
func (oc *Orchestrator) buildTxOpts(chainID *big.Int, ecClient *ec.Client) (*bind.TransactOpts, error) {
	...
	var err error
	if chainID.Cmp(oc.subchainID) == 0 {
